...
module.exports.help = {
  ...
  description: "Command ini berfungsi untuk membuat sebuah text channel untuk sementara."
}

exports.run = async (msg, args, creator) => {
  await msg.delete()
  if ((!msg.guild.me.permissions.has("SEND_MESSAGES") && !msg.guild.me.permissions.has("EMBED_LINKS")) || (!msg.guild.me.permissions.has("MANAGE_CHANNELS") && !msg.guild.me.permissions.has("VIEW_CHANNEL"))) return msg.reply("common.command.permissions.missing",{perm:"`SEND_MESSAGES`,`EMBED_LINKS`,`MANAGE_CHANNELS`,`VIEW_CHANNEL`"}).then(async (m) => { await  clear(m, 2000) });
  if (!args.length) return msg.reply(i18n.__("common.command.invalid")).then(async (m) => { await  clear(m, 2000) });
  if (args[0].toLowerCase() === "setup") {
	  if (msg.member.permissions.has("ADMINISTRATOR")) return create(msg, args, creator);
  ...
  if (args[0].toLowerCase() === "rename") {
	  var arguments = args
	  arguments.shift()
	  arguments = arguments.filter(item => item)
  ...
  if (args[0].toLowerCase() === "allowrole") {
	  ...
	  arguments = arguments.filter(item => item)
	  if(!arguments.length) return;
	  if(msg.member.permissions.has("ADMINISTRATOR")) return allowrole(msg, arguments, creator);
	  if(msg.guild.ownerId === creator.id) return allowrole(msg, arguments, creator);
	  return;
  };
}
...
async function rename(msg, args, creator) {
    ...
    if (topic === author) {
      const channel = msg.guild.channels.resolve(msg.channel.id);
	  if (!channel) return;
	  channel.setName(`👑-${nama.trim().toLowerCase()}`).then((c) => msg.channel.send(`:white_check_markk: Nama room di ganti ke **${c.name}**`)).catch(console.error);
    } else {
      return;
    }
}

...
function go(msg, player, isBot, botlayer, client) {
  ...
  var getlg = getlang("./src/assets/json/quiz");
...
function play(msg, participant, language, isBot, botlayer, client) {
  ...
  function display(rounds) {
    gameActive = true;
    msg.channel.send(embeds(i18n.__mf("games.quiz.command.ingame.question",{
        content: `\`${item[0].question.charAt(0).toUpperCase() + item[0].question.slice(1)}\``,
        players: getdata()
      }), { 
        text: i18n.__mf("games.quiz.command.ingame.rounds",{
        round:rounds,
        maxround:maxrounds
      }),
        url : TIMER_GIF
      ...
      messages.push(msg.id)
      if (isBot == true) {
        let botAction = [true,false];
        let posible = shuffle.pick(botAction, { 'picks' : 1 });
        let botAnswer = (item[0].answer.length > 1 ? item[0].answer[Math.floor(Math.random() * item[0].answer.length)] : item[0].answer[0]) || item[0].answer[0];
        if (posible == true) {
  ...
  function filter(m) {
    ...
    let answer = item[0].answer.some(obj => obj.toLowerCase() === m.content.toLowerCase());
    if (uuid && answer) {
  ...
  collector.on("collect", m => {
    ...
    if (item[0].answer.some(obj => obj.toLowerCase() == content)) {
      if(failed(author.id) === true) {
        m.reply(i18n.__("games.quiz.command.ingame.alreadydead"))
      } else {
        restart(author)
        collector.resetTimer()
      }
      ...
      if(failed(author.id) === true) {
        ...
        strike(author.id)
        m.reply(i18n.__mf("games.quiz.command.ingame.wronganswer",{
  ...
  collector.on("end", (collected, reason) => {
    ...
    switch (reason) {
      ...
      case 'round':
        ...
        msg.channel.send(embeds(i18n.__mf("games.quiz.command.ingame.gameover",{
